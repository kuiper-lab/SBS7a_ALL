...
for (vcf_path in vcf_vector) {
  ...
  if (length(colnames(sample_vcf)) > 2) {
    #Rename so it fits standard format
    ...
    sample_filtered_df <-
      sample_gonl_filtered_df[(apply(tumor_df[, alt_cols], 1, max) >= 5) &
                                ...
                                (apply(tumor_df[, vaf_cols], 1, max) >= 0.15), ]
    
    #Look at separate timepoints
    timepoints <-
      grep("D|([^C]R)", colnames(sample_vcf_gonl_filtered), value = T)
    for (timepoint in timepoints){
      #Find ref, alt and vaf columns
      ...
      df_read_based_filtered <- sample_filtered_df[sample_filtered_df[,read_count_alt_idx] > 0,]
      ...
      writeVcf(
        ...
        paste0(
          "~/Projects/hypermutated_ALL/RESULTS/filteredVcf_multipletimepoints/VEP105_nogermlinefilter_nosoftclippedbases_015/P_",
          timepoint,
          ".vcf"
        )
      )
      ...
      sample_mut_mat <- mut_matrix(list(rowRanges(sample_filtered_vcf_object)),
                                   ref_genome = "BSgenome.Hsapiens.UCSC.hg38")
      colnames(sample_mut_mat) <- timepoint
      ...
      mut_mat_s <- mut_matrix_stranded(rowRanges(sample_filtered_vcf_object),
                                       ref_genome,
                                       genes_hg38)
      colnames(mut_mat_s) <- timepoint
      strand_counts <- strand_occurrences(mut_mat_s)
      strand_counts$group <- timepoint
      ...
      write.csv(
        sample_mut_mat,
        paste0(
          "~/Projects/hypermutated_ALL/RESULTS/UnclusteredMutationMatrices_multipletimepoints_nogermlinefilter_nosoftclippedbases_015/P_",
          timepoint,
          ".csv"
        )
      )
  ...
  #Select patients with 2 timepoints
    ...
    #Add mutation matrix to larger matrix
    mut_mat_collection <- cbind(mut_mat_collection, sample_mut_mat)
    #Save mutation matrix
    write.csv(
      sample_mut_mat,
...
mut_mat_collection_UV_NoClusters_multipletimepoints_nogermlinefilter_nosoftclippedbases_015_19082024 <- mut_mat_collection
save(object = mut_mat_collection_UV_NoClusters_multipletimepoints_nogermlinefilter_nosoftclippedbases_015_19082024,
     file = "~/Projects/hypermutated_ALL/ANALYSES/deNovoExtraction/rdata/mut_mat_collection_UV_NoClusters_multipletimepoints_nogermlinefilter_nosoftclippedbases_nosoftclippedbases_015_19082024.rdata")
write.csv(mut_mat_collection_UV_NoClusters_multipletimepoints_nogermlinefilter_nosoftclippedbases_015_19082024,
          "~/Projects/hypermutated_ALL/ANALYSES/deNovoExtraction/rdata/mut_mat_collection_UV_NoClusters_multipletimepoints_nogermlinefilter_nosoftclippedbases_nosoftclippedbases_015_19082024.csv")

save(object = combined_strand_counts,
     file = "~/Projects/hypermutated_ALL/ANALYSES/melanoma_comparison/TranscriptionalStrandBias/ALLstrandcountsNoClusters_multipletimepoints_nogermlinefilter_nosoftclippedbases_nosoftclippedbases_015_19082024.rdata")
save(object = combined_strand_bias,
     file = "~/Projects/hypermutated_ALL/ANALYSES/melanoma_comparison/TranscriptionalStrandBias/ALLstrandbiasNoClusters_multipletimepoints_nogermlinefilter_nosoftclippedbases_nosoftclippedbases_015_19082024.rdata")
