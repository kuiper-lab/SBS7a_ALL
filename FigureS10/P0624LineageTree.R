...
library(ggsignif)
library(car)
source("~/Projects/SingleCellWGS/CODE/Scripts/fit_to_signatures_strict_tree_method.R")
source("~/Projects/SingleCellWGS/CODE/Scripts/plot_tree_contributions_branchnames.R")

#Make tree plot ----------------------------------------------------------------
#Load tree and vcf file
tree = readRDS("~/Projects/SingleCellWGS/DATA/CellPhyWrapper/P0624_includingbulktumor_MQandadjustedVAF025filtered_blacklistfiltered/TreeObject0.4.RDS")
vcf = VariantAnnotation::readVcf("~/Projects/SingleCellWGS/DATA/includebulktumor/P0624/MQandadjustedVAF025filtered/GenoType/P0624.output.snv.blacklistfiltered.vcf")
...
new = str_replace(old, "P0624", "P0624")
new[!grepl("P0624", new)] <- "Tumor Bulk"
tip_names_replace = data.frame(old, new)
tree_rename = change_tip_labels(tree = tree, id_change_tb = tip_names_replace)

#Plot
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624_Tree_SBS7a_MQandadjustedVAF025filtered_blacklistfiltered_includingbranchnames.pdf",
    width = 10)
plot_tree_contribution_branchnames(tree_rename, common_name = 'Lineage Tree',
                                   ...
                                   remove_min = 100)
dev.off()
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624_Tree_SBS1_MQandadjustedVAF025filtered_blacklistfiltered_includingbranchnames.pdf",
    width = 10)
plot_tree_contribution_branchnames(tree_rename, common_name = 'Lineage Tree',
                                   add_branch_length = T,
                                   ...
                                   remove_min = 100)
dev.off()
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624_Tree_PTA_MQandadjustedVAF025filtered_blacklistfiltered_includingbranchnames.pdf",
    width = 10)
plot_tree_contribution_branchnames(tree_rename, common_name = 'Lineage Tree',
                                   add_branch_length = T,
...
DP_df_large_branches_nobulk <- DP_df_large_branches[DP_df_large_branches$Cell != "P0624MSC" & DP_df_large_branches$Cell != "TumorBulk",]
...
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624_Quality.pdf",
    width = 10, height = 8)
plot_grid(mean_depth_plot, min_meandepthpercell_plot, sd_meandepthpercell_plot,
          median_depth_plot, min_mediandepthpercell_plot, sd_mediandepthpercell_plot,
          ncol = 2, byrow = F)
...
bulk_vaf_df$Branch <- factor(bulk_vaf_df$Branch,
                             levels = c("X", "V"))
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624SBS7aPositiveBranchesBulk.pdf",
    height = 4)
ggplot(bulk_vaf_df) +
...
#Add CNA information
bulk_vaf_df$CNA <- NA
bulk_vaf_df$CNA[bulk_vaf_df$chromosome == "6"] <- "p-gain+subclonal_gain" 
bulk_vaf_df$CNA[bulk_vaf_df$chromosome == "9"] <- "small_gain" 
bulk_vaf_df$CNA[bulk_vaf_df$chromosome %in% c("2", "12", "")] <- "LOH"
bulk_vaf_df$CNA[bulk_vaf_df$chromosome %in% c("4", "14", "17", "18", "21")] <- "gain"
bulk_vaf_df$CNA[bulk_vaf_df$chromosome %in% c("1", "3", "5", "7", "8", "10", "11",
                                              "13", "15", "16", "19", "20", "22")] <- "noCNA"
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624SBS7aPositiveBranchesBulk_CNAannotated.pdf",
    height = 4)
...
#Residuals are not normally distributed so we need to use nonparametric tests
wilcox.test(VAF ~ Branch, bulk_vaf_df_noCNA)

#Plot
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624SBS7aPositiveBranchesBulk_boxplot.pdf",
    height = 4, width = 5)
ggplot(bulk_vaf_df_noCNA,
       aes(x = Branch, y = VAF, fill = Branch)) +
  geom_boxplot() +
  geom_signif(comparisons = list(c("V", "X")), map_signif_level = T) +
  ylab("Allele Frequency In Tumor")
dev.off()

#Plot mutational profiles for branches
pdf("~/Projects/SingleCellWGS/RESULTS/SBS7a/Figures/P0624SBS7aPositiveBranchesMutationalProfile.pdf",
    height = 3)
